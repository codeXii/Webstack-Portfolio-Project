...
if(isset($_GET['property']) && isset($_GET['block'])){
    ...
    $block = filter_var($_GET['block'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    $count = filter_var($_GET['count'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);

    // var_dump($property);
    // die();

    $sql = "SELECT * FROM tenants WHERE property LIKE '$property%' AND block LIKE '%$block%' ORDER BY hn ASC";
...
<div class="main-panel bg-dark">
        <div class="content bg-dark">
            <div class="page-inner">
                <div class="mt-2 mb-4">
                    <h2 class="text-{{$text}} pb-2">LIST OF TENANTS IN <?= $property ?> <?= $block ?></h2>
                </div>
                <div>
                <form action="<?= ROOT_URL ?>listings.php" method="POST">
                        <div class="form-row">
                              <div class="form-group col-md-6">
                                  ...
                                  <input type="hidden" class="" name="count"  value="<?= $count ?>"  id="search" placeholder="Enter name">
